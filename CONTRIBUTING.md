...
This guide documents the local development workflow for contributors working on the MATO codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=mato`
...
POSTGRES_DB=mato
POSTGRES_PORT=5432
DATABASE_URL=postgres://mato:mato@localhost:5432/mato?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=mato_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://mato:mato@localhost:5432/mato_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../mato-feature -b feat/my-change main
cd ../mato-feature
make dev
```
...
The daemon authenticates using the CLI's stored token (`mato login`).
...
whatever server is configured in `~/.mato/config.json`. That's fine for
...
Example: worktree at `../mato-feat-auth` produces profile
`dev-mato_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `MATO_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.mato/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `mato` commands automatically use the same
...
rm -rf "$HOME/.mato/profiles/$PROFILE"
```
...
1. Compiles the `mato` CLI from `server/cmd/mato` into
   `apps/desktop/resources/bin/mato`
...
backend logs. If you set `MATO_DEV_VERIFICATION_CODE=888888` before starting
...
Nothing in this flow touches the system-installed `mato` or the default
`~/.mato/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.mato/config.json` | `~/.mato/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.mato/daemon.pid` | `~/.mato/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/mato_workspaces/` | `~/mato_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `mato_<slug>_<hash>` |
| Desktop profile | `desktop-api.mato.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U mato -d postgres -At -c "select datname from pg_database order by datname;"
```
