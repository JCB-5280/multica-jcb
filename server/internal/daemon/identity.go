...
// same machine shares one UUID stored at `~/.mato/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.mato/profiles/*/daemon.id` and returns every
...
// explicitly pins MATO_DAEMON_ID to the hostname itself, there's nothing
