...
// for the MATO Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: mato-agent <github@mato.ai>"
...
// path: a bare cache that already carries the MATO prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: mato-agent") {
...
// prepare-commit-msg hook that does NOT include the matoHookMarker
// sentinel — only the older `# Installed by the MATO daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no matoHookMarker line). Keeping a
...
# MATO: add Co-authored-by trailer for the MATO Agent.
# Installed by the MATO daemon. Do not edit — it will be overwritten.
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: mato-agent") {
...
// without the MATO marker (e.g. one a user added manually) must be left
