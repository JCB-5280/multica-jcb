...
// treats a hook as MATO-owned if its content contains ANY of these
...
var daemonInstalledHookSignatures = []string{
	matoHookMarker,
	"# Installed by the MATO daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the MATO Agent to every commit message.
...
// a Co-authored-by trailer for the MATO Agent. The hook is installed in the
...
// installed by the MATO daemon (current or any previously released
