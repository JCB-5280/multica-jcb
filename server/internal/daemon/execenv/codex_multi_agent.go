...
// subagents. The MATO daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the MATO runtime
...
//
// Users who explicitly want Codex native subagents inside a MATO task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `MATO_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// MATOCodexMultiAgentEnv is the env var users can set to keep Codex
...
const MATOCodexMultiAgentEnv = "MATO_CODEX_MULTI_AGENT"

// matoMultiAgentBeginMarker / matoMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	matoMultiAgentBeginMarker = "# BEGIN mato-managed multi-agent (do not edit; regenerated by daemon)"
	matoMultiAgentEndMarker   = "# END mato-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(matoMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(matoMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(MATOCodexMultiAgentEnv))
...
// renderMATOMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderMATOMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(matoMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(matoMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderMATOMultiAgentBlock(true)
...
//
// When MATO_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per MATO_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderMATOMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
