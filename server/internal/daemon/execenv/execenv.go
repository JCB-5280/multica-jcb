...
// checked out on demand by the agent via `mato repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string            // base path for all envs (e.g., ~/mato_workspaces)
...
// on demand via `mato repo checkout <url>`.
