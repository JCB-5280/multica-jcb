...
// `no such host` errors when calling out (for example, `mato issue get`
// hitting the MATO API). See upstream issue openai/codex#10390.
//
...
// actually reach the MATO API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the MATO CLI can reach the API.
...
// matoManagedBeginMarker / matoManagedEndMarker delimit the block the
...
const (
	matoManagedBeginMarker = "# BEGIN mato-managed (do not edit; regenerated by daemon)"
	matoManagedEndMarker   = "# END mato-managed"
)

// renderMATOManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.mato]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertMATOManagedBlock), avoids both traps.
func renderMATOManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(matoManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(matoManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(matoManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(matoManagedEndMarker) + `\n*`)

// upsertMATOManagedBlock returns the config content with the mato-managed
...
// `[permissions.mato]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderMATOManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertMATOManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderMATOManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the mato-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertMATOManagedBlock(existing, policy)
