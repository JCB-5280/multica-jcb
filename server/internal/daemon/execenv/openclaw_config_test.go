...
// MUL-2219 DoD calls out: the directory MATO writes skills into MUST be
...
func TestPrepareOpenclawSkillWriteMatchesScanPath(t *testing.T) {
	...
	for _, s := range skills {
		...
		if _, err := os.Stat(want); err != nil {
			t.Errorf("openclaw scan target %s missing — MATO's write path and the openclaw scanner are out of sync: %v", want, err)
		}
	}
}
