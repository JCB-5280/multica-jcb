...
const defaultCLIConfigPath = ".mato/config.json"
...
// An empty profile returns the default path (~/.mato/config.json).
// A named profile returns ~/.mato/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".mato", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.mato/. A named profile returns ~/.mato/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".mato"), nil
	}
	return filepath.Join(home, ".mato", "profiles", profile), nil
}
