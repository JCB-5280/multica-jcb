...
// the mato CLI version reported by the daemon at registration time. The
...
// `mato issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("mato CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("mato CLI version is below required minimum")
)
