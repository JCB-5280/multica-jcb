...
import { ApiError } from "@mato/core/api";
...
vi.mock("@mato/ui/components/ui/sidebar", () => ({
...
vi.mock("@mato/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@mato/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@mato/core/api", async (importOriginal) => ({ ...(await importOriginal<typeof import("@mato/core/api")>()), api: {} }));
vi.mock("@mato/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@mato/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@mato/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@mato/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@mato/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@mato/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@mato/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@mato/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@mato/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
