...
import type { ImportStarterIssuePayload } from "@mato/core/api";
...
export const PROJECT = {
  ...
  description:
    "A few things to try in MATO. Work through them at your own pace.",
};
...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[Learn how MATO works →](https://mato.ai/docs/how-mato-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in MATO**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to MATO — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using MATO. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to MATO — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using MATO. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to MATO — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using MATO. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to MATO — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using MATO. They're **exploring** what MATO can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to MATO — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using MATO. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to MATO — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using MATO.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in MATO are triggered when an issue has:`,
        ``,
        ...
        ``,
        `[Learn about assigning issues →](https://mato.ai/docs/assigning-issues)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about chat →](https://mato.ai/docs/chat)`,
      ].join("\n"),
    },
    {
      status: "todo",
      ...
      description: [
        ...
        ``,
        `[Learn about workspaces →](https://mato.ai/docs/workspaces)`,
      ].join("\n"),
    },
  ];
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `MATO works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
        ...
        ``,
        `[Learn about members and roles →](https://mato.ai/docs/members-roles)`,
      ].join("\n"),
    });
  }
  ...
  tier2.push({
    ...
    description: [
      ...
      ``,
      `[Learn about creating agents →](https://mato.ai/docs/agents-create)`,
    ].join("\n"),
  });
  ...
  const tier3: ImportStarterIssuePayload[] = [
    ...
    },
    {
      ...
      description: [
        ...
        ``,
        `[Learn about tasks →](https://mato.ai/docs/tasks)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about the inbox →](https://mato.ai/docs/inbox)`,
      ].join("\n"),
    },
    {
      status: "backlog",
      ...
      description: [
        ...
        ``,
        `[Learn about autopilots →](https://mato.ai/docs/autopilots)`,
      ].join("\n"),
    },
  ];
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        `1. Go to [github.com/mato.ai/mato/releases/latest](https://github.com/mato.ai/mato/releases/latest) and download the \`.dmg\` for macOS`,
        ...
        ``,
        `[Learn about runtimes →](https://mato.ai/docs/daemon-runtimes)`,
      ].join("\n"),
    },
    {
      status: "todo",
