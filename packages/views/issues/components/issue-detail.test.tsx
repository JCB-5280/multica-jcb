...
import type { Issue, TimelineEntry } from "@mato/core/types";
...
vi.mock("@mato/ui/hooks/use-mobile", () => ({
  useIsMobile: () => mockViewport.isMobile,
}));

// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @mato/core/hooks.tsx). vi.mock("@mato/core/paths") only intercepts
...
// Mock @mato/core/workspace/hooks
vi.mock("@mato/core/workspace/hooks", () => ({
  useActorName: () => ({
...
// Mock @mato/core/paths — after the URL-driven workspace refactor,
...
vi.mock("@mato/core/paths", async () => {
  const actual = await vi.importActual<typeof import("@mato/core/paths")>(
    "@mato/core/paths",
  );
  return {
    ...actual,
...
vi.mock("../../navigation", () => ({
  ...
  useNavigation: () => ({
    ...
    getShareableUrl: (p: string) => `https://app.mato.com${p}`,
  }),
...
// Mock issue config
vi.mock("@mato/core/issues/config", () => ({
...
vi.mock("@mato/core/issues/stores", () => ({
...
// Mock react-resizable-panels (used by @mato/ui/components/ui/resizable)
