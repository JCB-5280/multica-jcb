...
[mul-963]: https://mato-api.copilothub.ai/issues/28c34ad2-102a-4f46-91ac-336ed78c5859
...
If you see `no such host` *inside a Codex session on macOS* but `curl https://mato-api.copilothub.ai` from a plain shell on the same machine works, you are hitting the Seatbelt bug below.
...
The daemon writes a *mato-managed* block into each task's
`$CODEX_HOME/config.toml`, delimited by `# BEGIN mato-managed` /
`# END mato-managed` markers. Anything outside the markers is left
...
`[permissions.mato]`, a bare `sandbox_mode = "..."` line would be parsed
as `permissions.mato.sandbox_mode` and Codex would silently ignore it.
...
```bash
# Is the MATO API reachable at all?
curl -sSf https://mato-api.copilothub.ai/healthz
```

From inside a Codex session (after the daemon writes its config):

```bash
mato issue list --limit 1 --output json >/dev/null && echo OK
```
...
- **A. Domain-scoped `permissions` profile** (tight): when the upstream
  `network_access` fix is available, prefer writing a `permissions.mato`
  profile that allows only `mato-api.copilothub.ai` and
  `mato-static.copilothub.ai`. Keeps filesystem sandbox intact.
...
sed -n '/# BEGIN mato-managed/,/# END mato-managed/p' \
  ~/mato_workspaces/$WORKSPACE_ID/$TASK_SHORT/codex-home/config.toml
```
