...
<Callout type="info">
**Cloud runtimes are coming**, currently waitlist-only. Once live, you won't need a local daemon — agent tasks will execute on Multica Cloud directly. Sign up on the [Downloads](https://mato.ai/download) page to get notified.
</Callout>
...
<NumberedSteps>
  <Step number="01" title="Start with the runtime model">
    [How Multica works](/how-mato-works) — 30 seconds to read, and it settles the "server doesn't run agents, agents run on your machine" point once and for all.
  </Step>
