...
If you write MATO code, change a translation, or write Chinese product copy, this is the page to reference.
...
| `packages/ui` | nothing | `@mato/core`, business logic |
...
MATO's product nouns split into two categories:

- **Entity** — has a URL, a database row, an API type. In Chinese text, render as **lowercase English** so it visually reads like a type name and signals "this is a MATO system entity".
...
`issue` / `skill` / `task` are MATO's core entities. They have schema columns, API fields, and product UI labels that are all English. In Chinese text, they follow a **mixed rule** — what to use depends on where the word appears:
...
- **`skill`**: MATO-specific concept with no established Chinese term.
...
| Category | Terms |
| --- | --- |
| Brands | **MATO**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
