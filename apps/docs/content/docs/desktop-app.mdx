...
Grab the installer for your platform from the [Multica downloads page](https://mato.ai/download):
...
<Callout type="info">
**Desktop defaults to Multica Cloud, but can be pointed at a self-hosted instance with a local config file.** There is still no in-app "connect to self-host" picker. Desktop reads `~/.mato/desktop.json` before the renderer starts; if the file is missing, it uses the Cloud defaults.
