const RELEASE_ARCHIVE_PREFIX = "mato-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `mato-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `mato_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `mato_${os}_${mappedArch}.${ext}`;
