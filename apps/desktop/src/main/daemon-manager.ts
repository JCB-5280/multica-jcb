...
const PREFS_PATH = join(homedir(), ".mato", "desktop_prefs.json");
...
// Keep the Go impl in sync: server/cmd/mato/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".mato", "profiles", profile)
    : join(homedir(), ".mato");
}
...
// Sidecar file that records which MATO user the cached PAT in config.json
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["mato.exe"] : ["mato"];
 ...
 *   to `apps/desktop/resources/bin/mato`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<MATO.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "mato.exe" : "mato";
...
/**
 * Returns a usable `mato` binary path. Priority:
 ...
 *   5. `mato` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "MATO Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, MATO_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "mato CLI is not installed" };
