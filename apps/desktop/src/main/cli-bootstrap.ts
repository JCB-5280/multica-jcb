...
// Desktop prefers the bundled `mato` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/mato.ai/mato/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "mato.exe" : "mato";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `mato-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `mato` binary. If one is already present at
