...
import { ModalRegistry } from "@mato/views/modals/registry";
import { AppSidebar } from "@mato/views/layout";
...
import { WorkspaceSlugProvider, paths, useCurrentWorkspace } from "@mato/core/paths";
import { getCurrentSlug, subscribeToCurrentSlug } from "@mato/core/platform";
import { useDesktopUnreadBadge } from "@mato/views/platform";
...
function useInternalLinkHandler() {
  useEffect(() => {
    ...
    window.addEventListener("mato:navigate", handler);
    return () => window.removeEventListener("mato:navigate", handler);
  }, []);
}
  ...
  useEffect(() => {
    return window.desktopAPI.onInboxOpen(({ slug, issueKey }) => {
      ...
      window.dispatchEvent(
        new CustomEvent("mato:navigate", { detail: { path: inboxPath } }),
      );
    });
  }, []);
