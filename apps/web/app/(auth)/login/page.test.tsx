...
import enCommon from "@mato/views/locales/en/common.json";
import enAuth from "@mato/views/locales/en/auth.json";
import enSettings from "@mato/views/locales/en/settings.json";
...
vi.mock("@mato/core/auth", async () => {
  const actual =
    await vi.importActual<typeof import("@mato/core/auth")>(
      "@mato/core/auth",
    );
...
describe("LoginPage", () => {
  ...
  it("renders login form with email input and continue button", () => {
    ...
    expect(screen.getByText("Sign in to MATO")).toBeInTheDocument();
  ...
  it("calls sendCode with email on submit", async () => {
    ...
    await user.type(screen.getByLabelText("Email"), "test@mato.ai");
    await user.click(screen.getByRole("button", { name: "Continue" }));

    await waitFor(() => {
      expect(mockSendCode).toHaveBeenCalledWith("test@mato.ai");
    });
  });
  ...
  // the mato:// deep link, not silently redirect to the workspace page.
  it("mints a token and deep-links to Desktop when already logged in with platform=desktop", async () => {
    ...
    authStateRef.state.user = { id: "u1", email: "test@mato.ai" };
    ...
    try {
      ...
      await waitFor(() => {
        expect(hrefSetter).toHaveBeenCalledWith(
          "mato://auth/callback?token=handoff-jwt",
        );
      });
      expect(
        await screen.findByRole("button", { name: "Open MATO Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
