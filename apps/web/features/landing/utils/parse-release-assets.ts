 ...
 *   mato-desktop-{version}-mac-{arch}.{dmg|zip}
 *   mato-desktop-{version}-windows-{arch}.exe
 *   mato-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^mato-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
